...
All mixpanel events are summarized below. They are organized in sections which consists with a bold heading, indicating in which controller are there these events, and events groups of this controller. Events are grouped based on the methods or functions they belong to. Usually, there will also be some descriptions about the controller and the method.

---

In `HomeViewController` screen when user presses the menu button on the Navigation Bar, `MenuViewController` shows up. The User could then select any button on the controller view to segue to any of these controllers below.
...
This controller is the controller for user to recreate new avatars. However, when user creates his first avatar, the controller will be presented is `SetInformationViewController` and there is no mixpanel event being tracked in this controller.
